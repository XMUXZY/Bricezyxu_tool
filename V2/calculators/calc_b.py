...
def calc_max_level(start_level: int, mat1_type: int, mat1_lowest: float, mat2_count: float):
    """
    根据材料量计算可达成淬炼等级。

    参数:
        start_level: 初始淬炼等级
        mat1_type: 材料1类型索引 (0-3)
        mat1_lowest: 材料1换算为最低级的数量 (float('inf') 表示无限)
        mat2_count: 材料2（蚀日之晶）数量 (float('inf') 表示无限)

    返回:
        dict: {
            "current_level": 最终可达等级,
            "used_mat1_lowest": 消耗的最低级材料1总量,
            "used_mat2": 消耗的材料2总量,
            "details": [消耗明细列表],
        }
    """
    ...
    return {
        ...
        "used_mat1_lowest": used_mat1_lowest,
        "used_mat2": used_mat2,
        "details": details,
    }


def calc_materials_for_target(start_level: int, target_level: int):
    """
    根据目标等级计算所需材料。
    ...
    返回:
        dict: {
            "total_mat1_lowest": 所需最低级材料1总量,
            "total_mat2": 所需材料2总量,
            "mat1_type_usage": {类型索引: 该类型原始使用量},
            "highest_mat1_type": 最高材料类型索引,
        }
    """
    ...
    return {
        "total_mat1_lowest": total_mat1_lowest,
        "total_mat2": total_mat2,
        "mat1_type_usage": mat1_type_usage,
        "highest_mat1_type": highest_mat1_type,
    }
